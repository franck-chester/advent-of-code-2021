...
async function main() {
    try {
        ...
        for (call of bingoCall) {
            ...
            if (bingo) {
                console.log(`BINGO AFTER matchingBoard ${boardId} for ${call} of ${JSON.stringify(boards[boardId].matched)}`);
                console.log(`MatchingBoard ${boardId} for ${call}:\n${JSON.stringify(boards[boardId])}`);
                let sum = boards[boardId].matched.reduce(function (a, b) {
                    return a + parseInt(b);
                }, 0);
